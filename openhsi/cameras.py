...
# %% auto #0
__all__ = ['WebCamera', 'switched_camera', 'FlirCameraBase', 'FlirCamera', 'SharedFlirCamera', 'LucidCameraBase', 'LucidCamera', 'SharedLucidCamera', 'SimulatedCameraBase', 'SimulatedCamera', 'SharedSimulatedCamera', 'XimeaCameraBase', 'XimeaCamera', 'SharedXimeaCamera', 'LucidCameraBase', 'LucidCamera', 'SharedLucidCamera', 'FlirCameraBase', 'FlirCamera', 'SharedFlirCamera', 'LucidCameraBase', 'LucidCamera', 'SharedLucidCamera', 'XimeaCameraBase', 'XimeaCamera', 'SharedXimeaCamera', 'SimulatedCameraBase', 'SimulatedCamera', 'SharedSimulatedCamera']

# %% ../nbs/api/cameras/cameras.ipynb #f766f622
...
# %% ../nbs/api/cameras/cameras.ipynb #77f1a488
@delegates()
...
# %% ../nbs/api/cameras/cameras.ipynb #b3062efd
def switched_camera(
...
# %% ../nbs/api/cameras/flir.ipynb #5955dd34
...
# %% ../nbs/api/cameras/flir.ipynb #56beaadb
...
# %% ../nbs/api/cameras/flir.ipynb #be6e9b6d
@delegates()
class FlirCameraBase():
    """Interface for FLIR camera

        Any keyword-value pair arguments must match the those available in settings file. FlirCamera expects the ones listed below:
...
# %% ../nbs/api/cameras/flir.ipynb #ddc2c70a
@delegates()
class SharedFlirCamera(FlirCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/lucidvision.ipynb #a1cd16c3
@delegates()
...
# %% ../nbs/api/cameras/lucidvision.ipynb #71dd4649
@delegates()
class SharedLucidCamera(LucidCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/simulated.ipynb #bd8f06a6
@delegates()
...
# %% ../nbs/api/cameras/simulated.ipynb #c294b1c3
@delegates()
...
# %% ../nbs/api/cameras/simulated.ipynb #228232bf
@delegates()
class SharedSimulatedCamera(SimulatedCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/ximea.ipynb #5889b4fe
@delegates()
...
# %% ../nbs/api/cameras/ximea.ipynb #5b24569e
@delegates()
class SharedXimeaCamera(XimeaCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/lucidvision.ipynb #031f071f
@delegates()
class LucidCameraBase():
    """Core functionality for Lucid Vision Lab cameras
        
        Any keyword-value pair arguments must match the those available in settings file. LucidCamera expects the ones listed below:
...
# %% ../nbs/api/cameras/lucidvision.ipynb #a07857fb
@delegates()
class SharedLucidCamera(LucidCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/flir.ipynb #eb4db353
def set_camera_attribute(camera, attribute_name, value, alternatives=None, required=True):
    """
    Set a camera attribute with support for alternative attribute names.
...
# %% ../nbs/api/cameras/flir.ipynb #ad138d01
def get_min_exposure(camera):
    """
    Get minimum exposure time in microseconds with graceful fallback.
...
# %% ../nbs/api/cameras/flir.ipynb #709dc300
@delegates()
class FlirCameraBase():
    """Interface for FLIR camera
        ...
        - `exposure_ms`: is the camera exposure time to use
        - `pixel_format`: format of pixels readout sensor, ie Mono8, Mono10, Mono10p, Mono10Packed, Mono12, Mono12p, Mono12Packed, Mono16
        - `use_trigger`: set True to enable hardware trigger mode (default False)
        - `trigger_source`: trigger input line, e.g. "Line3" (default), "Line0", "Line1", "Software"
        - `trigger_activation`: edge polarity, "RisingEdge" (default) or "FallingEdge"
        - `trigger_timeout_ms`: milliseconds to wait for a trigger before error (default 5000)
    """
    ...
    def __init__(self, **kwargs):
        ...
        # Configure hardware trigger if requested
        if self.settings.get("use_trigger", False):
            trigger_source     = self.settings.get("trigger_source", "Line3")
            trigger_activation = self.settings.get("trigger_activation", "RisingEdge")
            # Must disable trigger before changing source/activation
            set_camera_attribute(self.flircam, "TriggerMode", "Off")
            set_camera_attribute(self.flircam, "TriggerSelector", "FrameStart")
            set_camera_attribute(self.flircam, "TriggerSource", trigger_source)
            set_camera_attribute(self.flircam, "TriggerActivation", trigger_activation)
            set_camera_attribute(self.flircam, "TriggerMode", "On")
...
# %% ../nbs/api/cameras/flir.ipynb #b445616d
@delegates()
class SharedFlirCamera(FlirCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/lucidvision.ipynb #4ed84a3c
@delegates()
class LucidCameraBase():
    """Core functionality for Lucid Vision Lab cameras
        ...
        - `mac_addr`: str = "1c:0f:af:01:7b:a0",
        - `use_trigger`: set True to enable hardware trigger mode (default False)
        - `trigger_source`: trigger input line, e.g. "Line5" (default), "Line0", "Line1", "Software"
        - `trigger_activation`: edge polarity, "RisingEdge" (default) or "FallingEdge"
        - `trigger_timeout_ms`: milliseconds to wait for a trigger before error (default 5000)
    """
    def __init__(self,**kwargs):
        ...
        # Configure hardware trigger if requested
        if self.settings.get("use_trigger", False):
            trigger_source     = self.settings.get("trigger_source", "Line5")
            trigger_activation = self.settings.get("trigger_activation", "RisingEdge")
            # Must disable trigger before changing source/activation
            self.deviceSettings["TriggerMode"].value      = "Off"
            self.deviceSettings["TriggerSelector"].value  = "FrameStart"
            self.deviceSettings["TriggerSource"].value    = trigger_source
            self.deviceSettings["TriggerActivation"].value = trigger_activation
            self.deviceSettings["TriggerMode"].value      = "On"
    ...
    def get_img(self) -> np.ndarray:
        timeout_ms = self.settings.get("trigger_timeout_ms", 5000)
...
# %% ../nbs/api/cameras/lucidvision.ipynb #75a4f8c5
@delegates()
class SharedLucidCamera(LucidCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/ximea.ipynb #0cbfd2e6
@delegates()
class XimeaCameraBase():
    ...
    """Core functionality for Ximea cameras
        ...
        - `use_trigger`: set True to enable hardware trigger mode (default False)
        - `trigger_source`: xiapi trigger source string, e.g. "XI_TRG_EDGE_RISING" (default), "XI_TRG_EDGE_FALLING", "XI_TRG_EDGE_ANY", "XI_TRG_SOFTWARE"
        - `trigger_port`: GPI port number for the hardware trigger input, e.g. 1 (default)
        - `trigger_timeout_ms`: milliseconds to wait for a trigger before error (default 5000)
    """
    # https://www.ximea.com/support/wiki/apis/Python
    def __init__(self, exposure_ms:float = 10, serial_num:str = None, **kwargs):
        ...
        # Configure hardware trigger if requested
        if self.settings.get("use_trigger", False):
            trigger_source = self.settings.get("trigger_source", "XI_TRG_EDGE_RISING")
            trigger_port   = self.settings.get("trigger_port", 1)
            timeout_ms     = self.settings.get("trigger_timeout_ms", 5000)
            self.xicam.set_gpi_selector(trigger_port)
            self.xicam.set_gpi_mode("XI_GPI_TRIGGER")
            self.xicam.set_trigger_source(trigger_source)
            self.xicam.set_timeout(timeout_ms)
...
# %% ../nbs/api/cameras/ximea.ipynb #98aebb86
@delegates()
class SharedXimeaCamera(XimeaCameraBase, SharedOpenHSI):
    pass

# %% ../nbs/api/cameras/simulated.ipynb #4e3d324f
@delegates()
class SimulatedCameraBase():
    """Core functionality for simulated camera"""
    ...
    def __init__(self, 
        ...
        Extra kwargs for trigger simulation:
        - `use_trigger`: set True to simulate hardware trigger mode (default False)
        - `sim_trigger_interval_ms`: simulated inter-trigger interval in ms (default 100)
        """
        super().__init__(**kwargs)
        self.mode = mode
    ...
    def get_img(self) -> np.ndarray:
        ...
        if self.settings.get("use_trigger", False):
            import time
            time.sleep(self.settings.get("sim_trigger_interval_ms", 100) / 1000.0)
        if self.mode in ("HgAr","flat"):
            return next(self.gen)
        if self.rgb_buff.is_empty():
            self.rgb_buff.slots_left = 0 # make buffer full again
...
# %% ../nbs/api/cameras/simulated.ipynb #02106e52
@delegates()
class SimulatedCamera(SimulatedCameraBase, OpenHSI):
    """Simulated camera using an RGB image as input. Hyperspectral data is produced using CIE XYZ matching functions."""
    pass

# %% ../nbs/api/cameras/simulated.ipynb #cbfd96d1
@delegates()
class SharedSimulatedCamera(SimulatedCameraBase, SharedOpenHSI):
    pass